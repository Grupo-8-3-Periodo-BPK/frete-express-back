...
class EmailControllerTest {
        ...
        @Test
        void testSendEmail_WithEmptyToField_ShouldFail() throws Exception {
                EmailController.EmailRequest emailRequest = new EmailController.EmailRequest();
                emailRequest.setTo(""); // Campo 'to' vazio
                emailRequest.setSubject("Assunto");
                emailRequest.setContent("Conteúdo");

                String requestJson = objectMapper.writeValueAsString(emailRequest);

                // Simular exceção quando 'to' é inválido
                doThrow(new IllegalArgumentException("O destinatário não pode ser nulo ou vazio"))
                                .when(emailService).sendSimpleEmail(eq(""), anyString(), anyString());
                ...
                mockMvc.perform(post("/api/email/send")
                                ...
                                .andExpect(content().string(
                                                "Erro ao enviar e-mail: O destinatário não pode ser nulo ou vazio"));

                verify(emailService, times(1)).sendSimpleEmail("", "Assunto", "Conteúdo");
        }

        @Test
        void testSendEmail_WithNullToField_ShouldFail() throws Exception {
                EmailController.EmailRequest emailRequest = new EmailController.EmailRequest();
                // 'to' é nulo por padrão
                emailRequest.setSubject("Assunto");
                emailRequest.setContent("Conteúdo");

                String requestJson = objectMapper.writeValueAsString(emailRequest);

                // Simular exceção quando 'to' é nulo
                doThrow(new IllegalArgumentException("O destinatário não pode ser nulo ou vazio"))
                                .when(emailService).sendSimpleEmail(isNull(), anyString(), anyString());
                ...
                verify(emailService, times(1)).sendSimpleEmail(null, "Assunto", "Conteúdo");
        }
